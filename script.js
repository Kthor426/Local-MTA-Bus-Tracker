  ...
  try {
    ...
    if (!stopData || stopData.length === 0) {
      ...
      filteredData.forEach((visit, index, array) => {
        ...
        const isAtTerminal = ["layover", "prevtrip", "firststop"].some((term) =>
          progressStatus.includes(term),
        );
        ...
        if (isAtTerminal && expectedArrival) {
          if (departureTime) {
            ...
            if (isAtTerminal && depDate > nextMinute) {
...
async function displayBusTimes() {
  ...
  try {
    ...
    northContainer.innerHTML =
      '<p style="color:red">Error loading northbound stops</p>';
    southContainer.innerHTML =
      '<p style="color:red">Error loading southbound stops</p>';
  }
}
